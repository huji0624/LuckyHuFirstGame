...
//		debugRender.render(mWorld, mSRender.getProjectionMatrix());
	...
	private static final int MaxBlock = 30;

	private void genBlock() {
//		if (blockNumber==2) {
//			return;
//		}
		while (mBlockTop - mOffset <= mStage.getHeight()) {
...
//		LHLevelLoader.instance().initLevel("level/level28.svg");
		...
		mBlockTop = Gdx.graphics.getHeight()/2;
		genBlock();
		// DEBUG
	}
...
//		mLabel.addAction(scoreMove);
		...
		label.setPosition(mStage.getWidth()/2 - label.getWidth()*label.getScaleX()/2,mStage.getHeight() / 2 + mPanel.getHeight() / 2 + label.getHeight()/2);
		mStage.addActor(label);
	}
